...
def series(minutes_back, value_fn, step=1):
    return {NOW - timedelta(minutes=m): value_fn(m) for m in range(0, minutes_back + 1, step)}
...
def make_ctx(project, now=NOW, weather=None, power=None, inverters=None, quoia=None, dc=None):
    client = MagicMock()
    for attr, value in [
        ("project_weather", weather),
        ("project_power", power),
        ("project_inverters", inverters),
        ("border_history", quoia),
        ("measurements_dc", dc),
    ]:
...
def weather_of(poa_series, temp_series=None, tmod_series=None):
    ...
    return WeatherSeries(
        irradiation={},
        irradiation_poa=poa_series,
        temperature=temp_series or {},
        temperature_poa=tmod_series,
        wind_speed={},
    )


def power_of(power_series, irr_series=None, spire=False):
    return PowerSeries(unit="kW", power=power_series, irradiance=irr_series or {}, spire=spire)
...
class TestPoaInvalid:
    def test_healthy_poa_is_ok(self, project):
        ctx = make_ctx(
            project,
            weather=weather_of(series(45, varying(800))),
    ...
    def test_negative_poa_fires(self, project):
        ctx = make_ctx(
            project,
            weather=weather_of(series(45, flat(-12))),
    ...
    def test_poa_zero_with_real_generation_fires(self, project):
        ctx = make_ctx(
            project,
            weather=weather_of(series(45, flat(0.0))),
    ...
    def test_frozen_poa_in_solar_hours_fires(self, project):
        ...
        ctx = make_ctx(
            project,
            weather=weather_of(series(60, flat(731.5))),
    ...
    def test_frozen_spire_model_is_ok(self, project):
        ...
        ctx = make_ctx(
            ...
            power=power_of(
                series(60, flat(400), step=5), irr_series=series(45, flat(606.0)), spire=True
            ),
        )
    ...
    def test_frozen_power_sensor_still_fires(self, project):
        ...
        ctx = make_ctx(
            ...
            weather=SolarViewNotAssociated("no estación"),
            power=power_of(
                series(60, flat(400), step=5), irr_series=series(45, flat(512.0)), spire=False
            ),
        )
    ...
    def test_frozen_at_night_freezes(self, project):
        ...
        ctx = make_ctx(project, now=NIGHT, weather=weather_of(night_series), power=power_of({}))
    ...
    def test_dawn_window_still_excluded(self, project):
        ...
        ctx = make_ctx(project, now=dawn, weather=weather_of(dawn_series), power=power_of({}))
...
class TestDataFrozen:
    def test_varying_power_is_ok(self, project):
        ctx = make_ctx(
            project,
            power=power_of(series(60, varying(300), step=5)),
            weather=SolarViewNotAssociated("no estación"),
        )
    ...
    def test_frozen_nonzero_power_fires(self, project):
        ctx = make_ctx(
            project,
            power=power_of(series(60, flat(415.3), step=5)),
            weather=SolarViewNotAssociated("no estación"),
        )
...
class TestPrInputsMissing:
    ...
    def test_all_inputs_present_is_ok(self, project):
        ctx = make_ctx(
            project,
            weather=weather_of(series(60, varying(700))),
            quoia=self.quoia_fresh(),
            dc=self.dc_fresh(),
    ...
    def test_missing_poa_fires_and_marks_interval(self, project):
        ...
        ctx.client.performance_ratio_historical.return_value = {}

        outcomes = PrInputsMissing().evaluate(ctx)

        assert outcomes[0].status == "firing"
        assert outcomes[0].evidence["missing_inputs"] == ["solarview_pr"]
        interval = NonComputableInterval.objects.get()
        assert interval.metric == "pr"
        assert interval.missing_inputs == ["solarview_pr"]

    def test_project_without_quoia_does_not_apply(self, project):
        ctx = make_ctx(
            project,
            weather=weather_of(series(60, varying(700))),
            quoia=SolarViewNotAssociated("sin medidor"),
            dc=self.dc_fresh(),
            power=power_of(series(60, flat(300), step=5)),
        )
        ctx.client.performance_ratio_historical.side_effect = SolarViewNotAssociated("sin PR")

        assert PrInputsMissing().evaluate(ctx) == []

    def test_night_is_ok(self, project):
        ctx = make_ctx(
            project, now=NIGHT, weather=weather_of({}), quoia={}, dc={}, power=power_of({})
        )

        assert PrInputsMissing().evaluate(ctx)[0].status == "not_computable"
    ...
    def test_interval_idempotent_same_hour(self, project):
        ...
        first = make_ctx(project, **ctx_kwargs)
        second = make_ctx(project, now=NOW + timedelta(minutes=5), **ctx_kwargs)
        first.client.performance_ratio_historical.return_value = {}
        second.client.performance_ratio_historical.return_value = {}
        PrInputsMissing().evaluate(first)
        PrInputsMissing().evaluate(second)
...
class TestAvailabilityInputsMissing:
    def inverters(self, time_ok=True, with_state=True, with_power=True):
        return [
            InverterLive(
                id=1571,
                dev_name="INV-1",
                state="Grid-connected" if with_state else None,
                power=100.0 if with_power else None,
                efficiency=98.0,
                temperature=60.0,
    ...
    def test_complete_inputs_ok(self, project):
        ctx = make_ctx(
            project, inverters=self.inverters(), weather=weather_of(series(30, varying(700)))
        )
    ...
    def test_silent_inverter_fires_and_marks(self, project):
        ...
        Inverter.objects.create(
            project=project, external_id=1571, dev_name="INV-1", synced_at=timezone.now()
        )
        ctx = make_ctx(
            project,
    ...
    def test_missing_poa_is_not_computable(self, project):
        ...
        ctx = make_ctx(
            ...
            weather=SolarViewNotAssociated("no estación"),
            power=power_of(series(30, flat(300), step=5)),
        )  # sin irradiance
    ...
    def test_night_does_not_evaluate(self, project):
        ctx = make_ctx(project, now=NIGHT, inverters=self.inverters(), weather=weather_of({}))
    ...
    def test_dusk_margin_does_not_evaluate(self, project):
        ...
        ctx = make_ctx(
            project, now=dusk, inverters=self.inverters(time_ok=False), weather=weather_of({})
        )
