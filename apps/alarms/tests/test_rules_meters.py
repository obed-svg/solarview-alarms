...
def quoia_series(deltas_kwh):
    """Contador acumulado de frontera: un punto cada 10 min hacia atrás desde NOW."""
    total = 1000.0
    points = {}
    for i, delta in enumerate(reversed(deltas_kwh)):
        ts = NOW - timedelta(minutes=10 * i)
        points[ts.strftime("%Y-%m-%d %H:%M:%S")] = {"value": round(total, 2), "unit": "kWh"}
        total -= delta
    return points
...
class TestMeterNoIncrement:
    def test_counter_increasing_is_ok(self, project):
        ctx = make_ctx(project, quoia=quoia_series([8, 8, 9, 8, 8, 8]))

        assert MeterNoIncrement().evaluate(ctx)[0].status == "ok"

    def test_flat_counter_with_generation_fires(self, project):
        ...
        assert outcomes[0].evidence["delta_kwh"] == 0

    def test_flat_counter_without_generation_is_ok(self, project):
...
class TestMeterInverterMismatch:
    # quoia_series con 6 puntos cada 10 min cubre 50 min → ΔE = 5 × delta_por_punto.
    ...
    def test_small_difference_is_ok(self, project):
        ...
        ctx = make_ctx(project, quoia=quoia_series([9.9] * 6), gen=generation([48, 50.0]))
    ...
    def test_above_5pct_fires_high(self, project):
        ...
        ctx = make_ctx(project, quoia=quoia_series([9.0] * 6), gen=generation([48, 50.0]))
    ...
    def test_between_3_and_5pct_fires_medium(self, project):
        ...
        ctx = make_ctx(project, quoia=quoia_series([9.6] * 6), gen=generation([48, 50.0]))
